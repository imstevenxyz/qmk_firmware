...
*/
 ...
 [_FN1_LAYER] = KEYMAP( /* Base */
    ...
    KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, MO(_FN1_LAYER), KC_NO, KC_TRNS
),
  /*
  * Layer _FN2_LAYER
  * ,-----------------------------------------------------------------------------------------.
 ...
 [_FN2_LAYER] = KEYMAP( /* Base */
    ...
    KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_TRNS, KC_NO, MO(_FN2_LAYER), KC_TRNS
 ),
};
...
layer_state_t layer_state_set_user(layer_state_t layer){
    switch(biton32(layer)){
        case _FN1_LAYER:
            setFNLayerColorOverride(1, 0xFF0000);
            break;
        case _FN2_LAYER:
            setFNLayerColorOverride(2, 0x00FF00);
            break;
        default:
            resetColorOverride();
            break;
    ...
    switch (keycode) {
        case KC_AP_LED_OFF:
            if (record->event.pressed)
                sendLedCommand(CMD_LED_OFF);
            return false;
        case KC_AP_LED_ON:
            if (record->event.pressed)
                sendLedCommand(CMD_LED_ON);
            return false;
        case KC_AP_LED_BRT_DOWN:
            if (record->event.pressed)
                sendLedCommand(CMD_LED_BRT_DOWN);
            return false;
        case KC_AP_LED_BRT_UP:
            if (record->event.pressed)
                sendLedCommand(CMD_LED_BRT_UP);
            return false;
        default:
            break;
    }
